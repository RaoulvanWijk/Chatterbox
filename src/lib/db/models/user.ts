...
export class User {
    ...
    const friends =
      (await this.client
        ...
        .innerJoin(
          users,
          or(
            and(eq(users.id, usersFriends.fromFriendId), ne(users.id, userId)),
            and(eq(users.id, usersFriends.toFriendId), ne(users.id, userId))
          )
        )
        .then((res) => res)) ?? null;
  ...
  async getUserFromUserFriendsId(userFriendsId: number, userId: number) {
    const user = await this.client
      .select({
        id: users.id,
        username: users.username,
      ...
      .from(usersFriends)
      .where(eq(usersFriends.id, userFriendsId))
      .innerJoin(
        users,
        or(
          and(eq(users.id, usersFriends.fromFriendId), ne(users.id, userId)),
